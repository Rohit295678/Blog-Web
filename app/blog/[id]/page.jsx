...
const BlogDetails = ({ params }) => {
  ...
  async function fetchBlog() {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/blog/${params.id}`
      );
  ...
  const handleLike = async () => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/blog/${params.id}/like`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${session?.user?.accessToken}`,
  ...
  const handleCommentSubmit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/blog/${params.id}/comment`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session?.user?.accessToken}`,
  ...
  const handleDeleteComment = async(commentId) => {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_HOST}/api/blog/${params.id}/comment/${commentId}`,
        {
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${session?.user?.accessToken}`,
